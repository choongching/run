// Brave's lion, the official single-path glyph filled with Brave's own
// gradient, inlined so no external asset is needed. Connector logos are the
// one sanctioned exception to the monochrome-icon rule, and this sits beside
// the full-colour Gmail and Drive marks, so a flat monochrome lion would have
// been the odd one out rather than the restrained one.
//
// Brave is not connectable: Pipedream reports the app with proxy_enabled
// false, so nobody can bring their own key. It has a face here anyway, because
// it is what every included search actually runs on and this page's job is to
// say what an agent runs on.
//
// The gradient id is namespaced. Two SVGs sharing a bare id like "a" on one
// page is a real bug: the second definition wins and the first icon renders
// with the wrong fill.
export function BraveIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" aria-hidden focusable="false" className={className}>
      <defs>
        <linearGradient id="run-brave-lion" x1="0" y1="0" x2="1" y2="1">
